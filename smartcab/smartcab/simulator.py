...
class Simulator(object):
    ...
    def __init__(self, env, size=None, update_delay=1.0, display=True):
        ...
        self.size = size if size is not None else ((self.env.grid_size[
                                                   0] + 1) * self.env.block_size, (self.env.grid_size[1] + 1) * self.env.block_size)
        ...
        for trial in xrange(n_trials):
            ...
            while True:
                try:
                    ...
                    # print "Simulator.run(): current_time =
                    # {:.3f}".format(self.current_time)
    ...
    def render(self):
        ...
        for agent, state in self.env.agent_states.iteritems():
            ...
            if state['destination'] is not None:
                self.pygame.draw.circle(self.screen, agent_color, (state['destination'][
                                        0] * self.env.block_size, state['destination'][1] * self.env.block_size), 6)
                self.pygame.draw.circle(self.screen, agent_color, (state['destination'][
                                        0] * self.env.block_size, state['destination'][1] * self.env.block_size), 15, 2)

        # * Overlays
        text_y = 10
        for text in self.env.status_text.split('\n'):
            self.screen.blit(self.font.render(text, True, self.colors[
                             'red'], self.bg_color), (100, text_y))
            text_y += 20
    ...
    def pause(self):
        ...
        self.screen.blit(self.font.render(pause_text, True, self.colors[
                         'cyan'], self.bg_color), (100, self.height - 40))
        self.pygame.display.flip()
        ...
        self.screen.blit(self.font.render(
            pause_text, True, self.bg_color, self.bg_color), (100, self.height - 40))
